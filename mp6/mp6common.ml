...
(****************** Support Infrastructure for  ML3 ***********************)
...
(***************** Adapted Infrstructure for MP3, CPS *********************)
let rec freeVarsInExp exp =
    ...
    | TryWithExp (e, i, he, hel) ->
      List.fold_left (fun vars (iopt, e) -> ((freeVarsInExp e) @ vars))
        ((freeVarsInExp e) @ (freeVarsInExp he))
        hel
...
(* The type of UpdateExnContCPS is really all wrong I think. It should
be basically a closure giving the environement for interpretting
the exp_cps *)

let rec freeVarsInExpCPS cont = (
    match cont with VarCPS (k, x) -> x :: freeVarsInContCPS k
    | ConstCPS (k, c) -> freeVarsInContCPS k
    | MonOpAppCPS (k,m,s,ke) ->
      s :: ((freeVarsInExnContCPS ke) @ (freeVarsInContCPS k))
    | BinOpAppCPS (k,b,r,s,ke) ->
      r :: s :: ((freeVarsInExnContCPS ke) @ (freeVarsInContCPS k))
    | IfCPS (r,e1,e2) -> r :: ((freeVarsInExpCPS e1) @ (freeVarsInExpCPS e2))
    | AppCPS (k,x1,x2,ke) ->
      x1::x2:: ((freeVarsInExnContCPS ke) @ (freeVarsInContCPS k))
    | FunCPS (k,x,Kvar,i,e) ->
      (freeVarsInContCPS k) @
        (List.filter (fun y -> not (x = y)) (freeVarsInExpCPS e))
    | FixCPS (k,f,x,Kvar,i,e) -> (freeVarsInContCPS k) @ 
...
and
   ...
   | ExnMatch ec -> freeVarsInExnContCPS  ec
 )
and
   freeVarsInExnContCPS ke = (
  match ke with ExnContVarCPS i -> []
    | EmptyExnContCPS -> []
    | UpdateExnContCPS (matching, ke) ->
      List.fold_left (fun vars (iopt, e) -> ((freeVarsInExpCPS e) @ vars))
      (freeVarsInExnContCPS ke)
      matching
    )
...
(*  Do I need this for exception continuations?
...
(*[[~ e]]k,ke = [[e]](FN r ke -> k (~ r)), ke, *)
   | MonOpAppExp (m, e) ->
      ...
      in cps_exp e ((FnContCPS (r, MonOpAppCPS (k, m, r, ke)))) ke
(*[[(e1 + e2)]]k,ke = [[e1]](FN r -> [[e2]](FN s -> k (r + s)),ke),ke*)
   | BinOpAppExp (b, e1, e2) ->
      ...
      let e2CPS =
       cps_exp e1 ((FnContCPS (v1, BinOpAppCPS(k, b, v1, v2, ke)))) ke in
      cps_exp e2 ((FnContCPS (v2, e2CPS))) ke
(*[[if e1 then e2 else e3]]k,ke = [[e1]]((FN r -> if r then [[e2]]k,ke else [[e3]]k,ke),ke*)
   | IfExp (e1,e2,e3) ->
      ...
      cps_exp e1 ((FnContCPS(r, IfCPS(r, e2cps, e3cps)))) ke
(*[[e1 e2]]k,ke = [[e1]](FN r -> [[e2]](FN s -> (r s k ke)),ke),ke*)
   | AppExp (e1,e2) -> 
      let v2 = freshFor (freeVarsInContCPS k @ freeVarsInExp e1) in
      let v1 = freshFor (v2 :: freeVarsInContCPS k) in
      let e1cps = cps_exp e1 (FnContCPS (v1, AppCPS(k, v1, v2, ke))) ke in
      cps_exp e2 (FnContCPS (v2, e1cps)) ke
(*[[fun x -> e]]k,ke = k(FUN x -> fn kx kes-> [[e]]kx,kes) *)
   | FunExp (x,e) ->
     let i = next_index() in
     let ecps = cps_exp e (ContVarCPS Kvar) (ExnContVarCPS i) in
...
(*[[let rec f x = e1 in e2]]k,ke =
  (FN f -> [[e2]]k,ke)(FIX f. FUN x -> fn k',ke' => [[e1]]k',ke')*)
   | LetRecInExp(f,x,e1,e2) ->
     let i = next_index() in
     let e1cps = cps_exp e1 (ContVarCPS Kvar) (ExnContVarCPS i)  in 
     let e2cps = cps_exp e2 k ke in 
     let fnk = FnContCPS(f,e2cps) in
     FixCPS(fnk,f,x,Kvar,i,e1cps)
...
(* [[raise e]]k,ke = [[e]](FN r -> match_exn r with ke), ke *)
   | RaiseExp e -> cps_exp e ((ExnMatch ke)) ke
...
let rec lookup_cont env k=
   match env with [] -> None
   | (bnd:: rem_env) ->
    (match bnd with ContBinding(k', c) -> Some c
     | _ -> lookup_cont rem_env k)
...
let rec string_of_value v =
  ...
  | ListVal l         -> "["^
                         (let rec pl = function
                              ...
                              in pl l)
  | CPSClosureVal (x, Kvar, i, e, m) ->  ("<some closure>")
  | CPSRecClosureVal (f, x, Kvar, i, e, m)  ->  ("<some recvar>")
